...
def render_multi_views_from_output(model_id, model_folder, category_views_root,
    ...
    # === Predicted mesh ===
    shape_path = model_folder / f"{model_id}.obj"  #MODIFIED
    tex_path = model_folder / f"{model_id}_ori.png"
...
def main():
    ...
    model_ids = [mid for mid in all_model_ids if (Path(pred_root) / mid / f"{mid}.obj").exists()]
...
# [RESULT] FID: 40.85397485770048
# [RESULT] KID: 0.005058438046499775

    '''
    python mv_render_uv_diff.py \
    --category Chair \
    --output_root /scratch/leuven/375/vsc37593/point-uv-diffusion-res \
