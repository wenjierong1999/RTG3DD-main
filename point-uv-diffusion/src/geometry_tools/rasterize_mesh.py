...
def load_uv_mesh(mesh_path, device):
    vertex_data = []
    face_data = []
    uv_vertex_data = []
    uv_face_data = []
    ...
    vertices = torch.from_numpy(np.array(vertex_data)).to(device)
    faces = torch.from_numpy(np.array(face_data)).to(device)
    uv_vertices = torch.from_numpy(np.array(uv_vertex_data)).to(device)
    uv_faces = torch.from_numpy(np.array(uv_face_data)).to(device)
...
if __name__=='__main__':
    ...
    # load mesh
    uv_mesh_dir = '/mnt/backup2/home/pgchen/lzz/soft_clean_data/shapenetV1/uv_model_old/03001627'
    camera_dir = '/mnt/backup2/home/pgchen/lzz/soft_clean_data/shapenetV1/render_image/camera/03001627'
    file_name = '100b18376b885f206ae9ad7e32c4139d'
    mesh_path = os.path.join(uv_mesh_dir, file_name, 'uv_texture_1024.obj')
    camera_path = os.path.join(camera_dir, file_name)
    '''
    ...
    pdb.set_trace()
    '''
    mesh = load_uv_mesh(mesh_path, device)
    ...
    cam_mv = align_blender_imgs(rotation_camera, elevation_camera, device)
