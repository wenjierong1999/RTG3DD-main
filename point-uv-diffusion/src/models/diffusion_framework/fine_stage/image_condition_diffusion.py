...
class DiffusionModule(BaseModule):
    ...
    def step(self, batch: Any):
        ...
        clip_condition = batch['input']['clip_condition'].to(device=self.device)
        clip_condition = clip_condition.squeeze(1)

        all_cond = {}
        all_cond['text'] = clip_condition
        all_cond['other_cond'] = cond

        return x, all_cond, smooth_map
