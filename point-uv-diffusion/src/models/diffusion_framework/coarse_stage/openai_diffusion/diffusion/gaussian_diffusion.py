...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        """
        if noise is None:
            if not self.offset_noise:
                noise = th.randn_like(x_start)
            else:
                noise = th.randn_like(x_start) + self.offset_noise*th.randn(x_start.shape[0], x_start.shape[1], 1)
    ...
    def training_losses(
        ...
        if noise is None:
            if not self.offset_noise:
                ...
                noise = th.randn_like(x_start) + self.offset_noise*th.randn((x_start.shape[0], x_start.shape[1], 1), device=x_start.device)
        ...
        # return terms # modified by xin yu
        return terms, model_output, target, x_t
