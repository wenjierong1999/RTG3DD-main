...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if se:
            self.se_layer = SE(self.out_channels)
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        qkv = self.qkv(self.norm(x).float()) # xin yu
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            # modified by xinyu
            #pdb.set_trace()
            num_res_block = num_res_blocks if isinstance(num_res_blocks, int) else num_res_blocks[level]
            dropout_level = dropout if isinstance(dropout, float) else dropout[level]
