...
def get_model_memory_usage(model, dtype=th.float32):
    param_size = 0
    dtype_size = th.tensor([], dtype=dtype).element_size()
    for param in model.parameters():
        param_size += param.numel() * dtype_size
    print(f"Model parameters occupy: {param_size / 1024**2:.2f} MB")
...
class Condition_Coarse_Fine_Model_Hybrid(UNetModel):
    def __init__(self, in_channels, cond_embed_ch, model_channels, *args, **kwargs):
        ...
        get_model_memory_usage(model=self)
