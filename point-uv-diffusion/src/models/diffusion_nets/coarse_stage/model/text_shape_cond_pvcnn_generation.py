...
'''
UV_PointNet(
  (fc1): Conv1d(7, 64, kernel_size=(1,), stride=(1,))
  (fc2): Conv1d(64, 128, kernel_size=(1,), stride=(1,))
  (fc3): Conv1d(128, 256, kernel_size=(1,), stride=(1,))
  (relu): ReLU()
  (fc_final): Conv1d(512, 64, kernel_size=(1,), stride=(1,))
)
'''
...
class Condition_Net(BaseNet):
    def __init__(self, text_embed_ch, cond_embed_ch, *args, **kwargs):
        ...
        self.shape_encoder = UV_PointNet(7, cond_embed_ch) # cond_embed_ch : 64
    ...
    def forward(self, x, t, fps_cond=None, shape_cond=None, text=None, **kwargs):
        assert shape_cond.size()[1] == 7 # 3 for pos, 3 for normal, 1 for mask
        shape_feat = self.shape_encoder(shape_cond) # [B, 64, 1]
        cond = torch.cat([fps_cond, shape_feat.repeat(1, 1, self.fps_num)], dim=1) # catenate fps and shape features

        inputs = x # 
        temb = self.embedf(self.get_timestep_embedding(t, inputs.device))[:, :, None].expand(-1, -1, inputs.shape[-1])

        # fuse image clip embeddings with time embedding
        if text!=None:
            text_embed = text.float()
            text_embed = self.silu(self.fc1(text_embed))
            text_embed = self.fc2(text_embed)
            # fusion using element-wise addition
        ...
        # encoder / save input features for skip connections
