...
parser.add_argument(
    '--synset', type=str, default='default_category',
    help='Category name or synset ID to organize output folders')
...
def direction_to_angles(dirs):
    rotation = np.degrees(np.arctan2(dirs[:, 0], dirs[:, 2])) % 360  # azimuth
    elevation = np.degrees(np.arcsin(dirs[:, 1]))                    # elevation
...
# /data/leuven/375/vsc37593/my_py_projects/point-uv-diffusion-draft/3D-Future-Demo/model_identifier/raw_model.obj


#model_identifier = os.path.split(os.path.split(args.obj)[0])[1]
model_identifier = os.path.basename(os.path.dirname(os.path.dirname(args.obj)))
#synset_idx = args.obj.split('/')[-3]

# img_follder = os.path.join(os.path.abspath(args.output_folder), 'img', model_identifier)
# camera_follder = os.path.join(os.path.abspath(args.output_folder), 'camera', model_identifier)

img_follder = os.path.join(os.path.abspath(args.output_folder), args.synset, 'img', model_identifier)
camera_follder = os.path.join(os.path.abspath(args.output_folder), args.synset, 'camera', model_identifier)

os.makedirs(img_follder, exist_ok=True)
os.makedirs(camera_follder, exist_ok=True)

# if args.views == 1:
#     rotation_angle_list = [-60]
#     elevation_angle_list = [30]
# else:
#     rotation_angle_list = np.random.rand(args.views)
#     #elevation_angle_list = np.random.rand(args.views)
#     elevation_angle_list = 2*np.random.rand(args.views)-1 #(-1,1)
...
if args.views == 1:
    ...
    dirs = sample_fibonacci_sphere(args.views)
    rotation_angle_list, elevation_angle_list = direction_to_angles(dirs)
